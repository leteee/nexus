...
from nexus import plugin, PluginConfig, DataSink
...
class DataGeneratorConfig(PluginConfig):
    ...
    random_seed: int = 42
    output_data: Annotated[str, DataSink(name="generated_data")] = "generated_data.csv"
...
class SampleDataGeneratorConfig(PluginConfig):
    ...
    output_data: Annotated[str, DataSink(name="sample_data")] = "sample_data.csv"
...
def generate_synthetic_data(config: DataGeneratorConfig, logger) -> pd.DataFrame:
    ...
    """
    logger.info(f"Generating {config.num_rows} rows with {config.num_categories} categories")
    ...
    data = {
        ...
        'category': np.random.choice(
            [f'Category_{i}' for i in range(config.num_categories)],
            config.num_rows
        ),
    ...
    if config.noise_level > 0:
        ...
        outlier_indices = np.random.choice(config.num_rows, size=outlier_count, replace=False)
        df.loc[outlier_indices, 'value'] *= np.random.choice([5, -3], len(outlier_indices))

    logger.info(f"Generated dataset: shape={df.shape}, memory={df.memory_usage(deep=True).sum() / 1024:.1f}KB")
    return df


@plugin(name="Sample Data Generator", config=SampleDataGeneratorConfig)
def generate_sample_dataset(config: SampleDataGeneratorConfig, logger) -> pd.DataFrame:
    """
...
def _generate_sales_data(num_rows: int, logger) -> pd.DataFrame:
    ...
    data = {
        ...
        'customer_type': np.random.choice(['Premium', 'Standard', 'Basic'], num_rows, p=[0.2, 0.5, 0.3])
    }
...
def _generate_product_data(num_rows: int, logger) -> pd.DataFrame:
    ...
    data = {
        ...
        'cost': lambda: data['price'] * np.random.uniform(0.3, 0.8, num_rows),
